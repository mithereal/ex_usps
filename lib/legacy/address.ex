defmodule UspsEx.Legacy.Address do
  @moduledoc """
  Represents an address that can be passed to other `UspsEx` functions. Do
  *not* initialize this struct directly. Instead, use `address/1`.
  ...
  If `name` is specified directly, UspsEx will try to infer the first and last
  ...
  def validate(%__MODULE__{} = address, _opts) do
    case address.country do
      "US" ->
        UspsEx.Legacy.validate_address(address)
  ...
  Returns the state code without its country code prefix.
      ...
      iex> address = UspsEx.Legacy.Address.state_without_country(%{
      ...>   first_name: "Earl",
      ...>   last_name: "Grey",
      ...>   phone: "123-123-1234",
      ...>   address: "9999 Hobby Lane",
