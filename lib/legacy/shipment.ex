defmodule UspsEx.Legacy.Shipment do
  @moduledoc """
  ...
  user's internal identifier for the shipment. The id is not used by UspsEx.Legacy.
  ...
  def new(from, to, packages \\ [], opts \\ []) do
    ...
    if allow_high_risk == nil do
      if Enum.member?(@high_risk_countries, String.to_atom(String.downcase(to.country))) do
        throw(
          {:error,
           "Usps flags #{to.country} as high risk, you must pass [high_risk: true] ex. UspsEx.Legacy.Shipment.new(from, to, packages, [high_risk: true]) ."}
        )
      end
    end
