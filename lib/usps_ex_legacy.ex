defmodule UspsEx.Legacy do
  ...
  alias UspsEx.Legacy.Error
  alias UspsEx.Legacy.Transaction

  import UspsEx.Legacy.Util,
    only: [
      price_to_cents: 1,
      international?: 1,
      create_hash: 1,
  ...
  alias UspsEx.Legacy.Client
  alias UspsEx.Legacy.Config
  alias UspsEx.Legacy.Error
  alias UspsEx.Legacy.Rate
  alias UspsEx.Legacy.Service
  alias UspsEx.Legacy.Package
  alias UspsEx.Legacy.Util
  ...
  def fetch_rates(shipment) do
    rates =
      UspsEx.Legacy.Service.services(shipment)
  ...
  def service_code(id), do: UspsEx.Legacy.Service.service_code(id)
  def insurance_code(id), do: UspsEx.Legacy.Insurance.code(id)
  ...
  def config() do
    with un when is_binary(un) <-
           Application.get_env(:legacy, :username, {:error, :not_found, :username}),
         pw when is_binary(pw) <-
           Application.get_env(:legacy, :password, {:error, :not_found, :password}) do
      %{
        username: un,
        password: pw
      }
